...
class TopRatedTVShowsBloc
  ...
  Future<void> _getTVShows(Emitter<TopRatedTVShowsState> emit) async {
    ...
    result.fold(
      (l) => emit(
        state.copyWith(status: GetAllRequestStatus.error, message: l.message),
      ),
      (r) {
        page++;
        emit(
  ...
  Future<void> _fetchMoreTVShows(
    FetchMoreTopRatedTVShowsEvent event,
    Emitter<TopRatedTVShowsState> emit,
  ) async {
    final result = await _getAllTopRatedTvShowsUseCase(page);
    result.fold(
